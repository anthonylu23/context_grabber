...
export const handleHostCaptureRequest = async (
  ...
  if (!supportsHostCaptureRequest(request)) {
    ...
    return createErrorMessage(errorCode, message, requestId, timestamp, false);
  }
  ...
  const payload = createBrowserPayload(extraction, browser);
  ...
  if (!sizeValidation.ok) {
    ...
    return createErrorMessage(issue.code, issue.message, request.id, timestamp, true);
  }

  return createCaptureResponseMessage(payload, request.id, timestamp);
};
